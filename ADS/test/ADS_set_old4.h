...
public:
    ...
    class Iterator{
        ...
        size_t index; // topindex indexInTab{set->useBucket(index).bucketTopIndex}
    ...
    public:
        ...
        explicit Iterator(value_type *pos, const  ADS_set<Key,N> *set): pos{pos}, set{set} {
            if (pos != set->virEndElem)
                index = set->useBucket(set->findIndex(*pos)).bucketTopIndex;
            }
        ...
        Iterator& operator++() {
            if (pos==set->virEndElem)
                return *this;
            //size_t index{set->findIndex(*pos)}; // save in privat
            size_t posInBuc{set->useBucket(index).posBucket(*pos)};
            // if next key in the same bucket
            if ( posInBuc+1 < set->useBucket(index).sizeBucket) {
                pos=&(set->useBucket(index).bucket[++posInBuc]);
                return *this;
            }
            // else search next bucket
            for (size_t i{index+1}; i < set->sizeTable; ++i)
                if (i == set->useBucket(i).bucketTopIndex && set->useBucket(i).sizeBucket !=0) {
        ...
        for (size_t l{0}; l < sizeTable; ++l)
            if (useBucket(l).sizeBucket !=0) {
                ...
                for(size_t m{0}; m < numbElem; m++){
                    ...
                    // if next key in the same bucket
                    if ( posInBuc+1 < useBucket(index).sizeBucket)
                        elm=useBucket(index).bucket[++posInBuc];
                    else{
                        // else search next bucket
                    
                        for (size_t i{index+1}; i < sizeTable; ++i)
                            if (i == useBucket(i).bucketTopIndex && useBucket(i).sizeBucket !=0) {
